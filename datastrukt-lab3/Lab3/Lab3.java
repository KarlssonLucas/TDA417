...
public class Lab3 {
    ...
    static ScapegoatTree<Ngram, ArrayList<Path>> buildIndex(ScapegoatTree<Path, Ngram[]> files) {
        ...
        Iterable<Path> paths = files.keys();
        for (Path p : paths) {
            for (Ngram ng : files.get(p)) {
               if (!index.contains(ng)) {
                   ArrayList<Path> currPaths = new ArrayList();
                   currPaths.add(p);
                   index.put(ng, currPaths);
               } else {
                   index.get(ng).add(p);
               }
            }
    ...
    static ScapegoatTree<PathPair, Integer> findSimilarity(ScapegoatTree<Path, Ngram[]> files, ScapegoatTree<Ngram, ArrayList<Path>> index) {
        ...
        for (Ngram ng : index.keys()) {
            for (Path p1 : index.get(ng)) {
                for (Path p2 : index.get(ng)) {
                    if (p1.equals(p2)) continue;
                    PathPair pair = new PathPair(p2, p1);
